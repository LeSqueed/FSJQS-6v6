...
const { generateHeroSettings } = require('./hero-settings-generator.js'); // Import the new module

const { version } = require('./package.json');

const SOURCE_DIR = path.resolve(__dirname) + path.sep + 'src' + path.sep;
const HERO_SETTINGS_FILE = path.join(__dirname, 'tools', 'heroes.json');
...
async function generateWorkshop() {
    try {
        ...
        const heroBlock = await generateHeroSettings(HERO_SETTINGS_FILE, LOBBY_FILE);

        // Read lobby.opy and replace the allTeams block
        let lobbyFileText = fs.readFileSync(LOBBY_FILE, 'utf8');
        const allTeamsStart = lobbyFileText.indexOf('"allTeams"');
        if (allTeamsStart !== -1) {
            const openBrace = lobbyFileText.indexOf('{', allTeamsStart);
            if (openBrace !== -1) {
                let braceCount = 1;
                let i = openBrace + 1;
                while (i < lobbyFileText.length && braceCount > 0) {
                    if (lobbyFileText[i] === '{') braceCount++;
                    else if (lobbyFileText[i] === '}') braceCount--;
                    i++;
                }
                if (braceCount === 0) {
                    const before = lobbyFileText.slice(0, openBrace + 1);
                    const after = lobbyFileText.slice(i - 1);                    
                    const indentedBlock = '\n' + heroBlock.split('\n').map(line => '            ' + line).join('\n') + '\n';
                    lobbyFileText = before + indentedBlock + after;
                    fs.writeFileSync(LOBBY_FILE, lobbyFileText);
                    console.log(colors.green(`Updated allTeams hero block in lobby.opy`));
                } else {
                    console.log(colors.red(`Could not find matching closing brace for allTeams block in ${LOBBY_FILE}`));
                }
            } else {
                console.log(colors.red(`Could not find opening brace for allTeams block in ${LOBBY_FILE}`));
            }
        } else {
            console.log(colors.yellow(`Warning: Could not find allTeams block to update in ${LOBBY_FILE}`));
        }

        console.log(colors.green(`Hero settings generated successfully from ${HERO_SETTINGS_FILE}`));

        let mainFileText = fs.readFileSync(MAIN_FILE, 'utf8');
        console.log(colors.green(`Compiling ${MAIN_FILE}...`));

        console.log(colors.green(`Compiling ${MAIN_FILE} with generated settings...`));
